...
__get_version_control_info() {
    local status="✓"
    if __is_dirty_repo; then
        status="✗"
    fi
    echo -en "${blue}git:(${wipe}${red}$(__get_git_branch)${wipe}${blue})${wipe} ${yellow}$status${wipe} "
}

__get_ruby_info() {
    echo -en "${red}⚡${wipe} ${blue}ruby:(${wipe}${red}$(__get_ruby_version)${wipe}${blue})${wipe} ${red}⚡${wipe} "
}
